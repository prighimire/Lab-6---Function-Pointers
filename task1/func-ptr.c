...
{
	//TODO: IMPLEMENT ME!
  const Process *process_1 = (const Process *)this;
  const Process *process_2 = (const Process *)that;

  if (process_1->priority!= process_2->priority){
    return process_2->priority - process_1->priority;
  } else {
    return process_1->arrival_time - process_2->arrival_time;
  }
}
